...
| The conclusion begins with one continuous, state-derived naturalized summary | Pure `buildNaturalizedSummary(pack, state)` | Completed-state gate, beat-local disclosure, typed atoms, and ordered paragraph rendering | Copy suite passed 31/31 on the current binding | Browser and assistive-technology renewal pending |
